...
// UI Design
function ProductCard(product, stock_info, addToCartButtonStyle) {
    return `
        <div class="product">
            ...
            <p>${stock_info}</p>
            <button class="product-button" style="${addToCartButtonStyle}" onclick="addToCart('${product['product_id']}')">Add to Cart</button>
        </div>
    `
}
...
async function showProducts(category) {
    ...
    try {
        ...
        products.forEach((product) => {
            let stock_info = product['in_stock'] > 0 ? "In stock" : "Not in stock";
            let addToCartButtonStyle = product['in_stock'] > 0 ? 'display: grid; text-align: center; box-sizing: border-box;' : 'display: none;';
            document.getElementById("product-grid").innerHTML += ProductCard(product, stock_info, addToCartButtonStyle);
        });
...
async function checkItemAvailability(productId) {
